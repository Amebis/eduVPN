...
WintunLogger(_In_ WINTUN_LOGGER_LEVEL Level, _In_z_ const WCHAR* LogLine)
{
...
EvaluateWintunPool(_In_ MSIHANDLE hInstall)
{
    ...
    if (iInstalled >= INSTALLSTATE_LOCAL && iAction > INSTALLSTATE_BROKEN && iAction < INSTALLSTATE_LOCAL)
    {
        ...
        // Schedule Wintun adapter pool deletition.
        PMSIHANDLE hRecord = MsiCreateRecord(1);
        if (!(MSIHANDLE)hRecord) {
            LOG_ERROR(TEXT("MsiCreateRecord failed"));
            return ERROR_SUCCESS;
        }
        uiResult = MsiRecordSetString(hRecord, 0, TEXT("\"[") WINTUN_DIRECTORY TEXT("]") WINTUN_FILE_NAME TEXT("\" \"[") WINTUN_POOL_PROPERTY_NAME TEXT("]\""));
        if (uiResult != ERROR_SUCCESS) {
            LOG_ERROR_NUM(uiResult, TEXT("MsiRecordSetString failed"));
            return ERROR_SUCCESS;
        }
        tstring sRemoveWintunPool;
        uiResult = MsiFormatRecord(hInstall, hRecord, sRemoveWintunPool);
        if (uiResult != ERROR_SUCCESS) {
            LOG_ERROR_NUM(uiResult, TEXT("MsiFormatRecord failed"));
            return ERROR_SUCCESS;
        }
        uiResult = MsiSetProperty(hInstall, WUNTUN_REMOVE_POOL_CA_NAME, sRemoveWintunPool.c_str());
        if (uiResult != ERROR_SUCCESS) {
            LOG_ERROR_NUM(uiResult, TEXT("MsiSetProperty(\"%s\") failed"), WUNTUN_REMOVE_POOL_CA_NAME);
            return ERROR_SUCCESS;
        }
    }
...
_Return_type_success_(return == ERROR_SUCCESS) UINT __stdcall
RemoveWintunPool(_In_ MSIHANDLE hInstall)
{
    s_hInstall = hInstall;
    ...
    if (nArgs < 2) {
        LOG_ERROR(TEXT("Syntax: <%s path> <pool name>"), WINTUN_FILE_NAME);
        goto cleanup_szArg;
    }
...
#define X(Name, Type) ((Name = (Type)GetProcAddress(hWintun, #Name)) == NULL)
    if (X(WintunSetLogger, WINTUN_SET_LOGGER_FUNC) ||
...
#undef X
    ...
    if (!WintunDeletePoolDriver(szArg[1], &bRebootRequired))
        LOG_ERROR_NUM(GetLastError(), TEXT("WintunDeletePoolDriver(\"%ls\") failed"), szArg[1]);
    if (bRebootRequired)
